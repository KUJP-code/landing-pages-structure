...
output_dir = "qr_codes/event_1/postcard"
...
def generate_qr_code(school_code, school_name):
    ...
    url = f"https://kids-up.jp/qr#ev1-{school_code}-pc"
    ...
    img_filename = f"{school_code}-{school_name}_event-1_window_qr.png"
    full_img_filename = os.path.join(output_dir, img_filename)
    
    # Save the PNG image
    img.save(full_img_filename)
    print(full_img_filename)
    
    # Append the filename (without path) to the text file
